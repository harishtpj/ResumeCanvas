...
const emit = defineEmits(['alert', 'run-optimizer', 'logout']);
</script>
  ...
  >
    ...
    <div class="p-3 mb-4 bg-slate-50/55 border border-slate-100/50 rounded-2xl">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-full bg-primary text-white font-bold flex items-center justify-center text-xs">
          {{ currentUser.name.slice(0, 2) }}
        </div>
        <div class="truncate">
          <p class="font-bold text-sm text-on-surface leading-snug">{{ currentUser.name }}</p>
          <p class="text-xs text-on-surface-variant font-medium">{{ currentUser.plan }}</p>
        </div>
      </div>
    </div>
    ...
    <nav class="flex-1 flex flex-col gap-1.5 font-sans">
      <span 
        @click="emit('alert', 'Dashboard Overview initialized.', 'info')"
      ...
      <span 
        @click="emit('alert', 'Loading My Templates library...', 'info')"
